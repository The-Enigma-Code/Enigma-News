...
export class NewsCopy extends Component {
 noOfNewsPerPage=6;
  constructor() {
    super();
    ...
    this.state = {
      ...
      page:1,
      activePage: null,
      startIndex:0,//from where slice start
      endIndex:this.noOfNewsPerPage,//where slice end
      // darkMode:{b}
    };
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (prevState.activePage) {
      console.log("previous:"+ prevState.activePage)
...
nextPageIndicator = () => {
  console.log("next handler invkoed")
    ...
    const ele = document.getElementById(`${parseInt(this.state.endIndex)/parseInt(this.noOfNewsPerPage)}`)
    ele.setAttribute('style', 'position: relative; bottom: 1vh;');
}
  nextHandler = () => {
  ...
  render() {
    ...
    for (let i = 1; i <= Math.ceil(this.state.totalResults / this.noOfNewsPerPage); i++) {
      if(i===1 && Math.ceil(this.state.totalResults / this.noOfNewsPerPage)>1){
        buttons.push(<button className='mx-1' key={i} id={i} data-value={i} onClick={this.pageNumberHandler} ref={this.firstPageRef}>{i}</button>);
      }
      else{

        buttons.push(<button className='mx-1' key={i} data-value={i} id={i} onClick={this.pageNumberHandler}>{i}</button>);
      }
    }
    /**
